...
def show_pcl(pcl):
    ...
    # With help from: https://knowledge.udacity.com/questions/801783 
    ...
    # With help from: https://knowledge.udacity.com/questions/817654
...
def show_range_image(frame, lidar_name):
    ...
    # Load range image (From Example C1-5-1)
    ...
    # map value range to 8bit (From Example C1-5-4)
    ...
    flat_list = []
    for sublist in ri_intensity:
        for item in sublist:
            flat_list.append(item)
    flat_list.sort()
    
    len_list = len(flat_list)
    hi_intensity = flat_list[int(np.floor(.99*len_list))]
    lo_intensity = flat_list[int(np.ceil(.01*len_list))]
    
    # map intensity to 8bit
    ri_intensity = ri_intensity * 255 / (hi_intensity - lo_intensity)
    ...
    # For stacked range and intensity image, use next line and comment the following one. For intensity image only, comment next line and uncomment following one.
    img_range_intensity = range_intensity.astype(np.uint8)    
    #img_range_intensity = img_intensity
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    # Used Exercise C2-3-2 : Transform metric point coordinates to BEV space
